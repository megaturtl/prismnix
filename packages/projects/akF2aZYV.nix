{lib, callPackage, ...}:
let
    versions = (let
        _nzKaGn0h = {
            "id" = "nzKaGn0h";
            "file" = "Cobweb Outline.zip";
            "hash" = "sha512-FJ9PmbbIUsg4WHDENXstpignCuQDl+ngUUnzMfsBy1dWstl3vSXpngu1hgSAqYuwcBjiyR0hXvZIDqffvZDx3g==";
        };
        _GqLacqjY = {
            "id" = "GqLacqjY";
            "file" = "Cobweb Outline.zip";
            "hash" = "sha512-FJ9PmbbIUsg4WHDENXstpignCuQDl+ngUUnzMfsBy1dWstl3vSXpngu1hgSAqYuwcBjiyR0hXvZIDqffvZDx3g==";
        };
        _w8ZNVjwr = {
            "id" = "w8ZNVjwr";
            "file" = "Cobweb Outline.zip";
            "hash" = "sha512-FJ9PmbbIUsg4WHDENXstpignCuQDl+ngUUnzMfsBy1dWstl3vSXpngu1hgSAqYuwcBjiyR0hXvZIDqffvZDx3g==";
        };
        _HtjGizUo = {
            "id" = "HtjGizUo";
            "file" = "Cobweb Outline.zip";
            "hash" = "sha512-FJ9PmbbIUsg4WHDENXstpignCuQDl+ngUUnzMfsBy1dWstl3vSXpngu1hgSAqYuwcBjiyR0hXvZIDqffvZDx3g==";
        };
        _mYnFHnfb = {
            "id" = "mYnFHnfb";
            "file" = "Cobweb Outline.zip";
            "hash" = "sha512-FJ9PmbbIUsg4WHDENXstpignCuQDl+ngUUnzMfsBy1dWstl3vSXpngu1hgSAqYuwcBjiyR0hXvZIDqffvZDx3g==";
        };
        _2WzmpetA = {
            "id" = "2WzmpetA";
            "file" = "Cobweb Outline.zip";
            "hash" = "sha512-FJ9PmbbIUsg4WHDENXstpignCuQDl+ngUUnzMfsBy1dWstl3vSXpngu1hgSAqYuwcBjiyR0hXvZIDqffvZDx3g==";
        };
        _QsSiZsU4 = {
            "id" = "QsSiZsU4";
            "file" = "Cobweb Outline.zip";
            "hash" = "sha512-FJ9PmbbIUsg4WHDENXstpignCuQDl+ngUUnzMfsBy1dWstl3vSXpngu1hgSAqYuwcBjiyR0hXvZIDqffvZDx3g==";
        };
        _VxgTa8Mp = {
            "id" = "VxgTa8Mp";
            "file" = "Cobweb Outline.zip";
            "hash" = "sha512-FJ9PmbbIUsg4WHDENXstpignCuQDl+ngUUnzMfsBy1dWstl3vSXpngu1hgSAqYuwcBjiyR0hXvZIDqffvZDx3g==";
        };
        _YwH559by = {
            "id" = "YwH559by";
            "file" = "Cobweb Outline.zip";
            "hash" = "sha512-FJ9PmbbIUsg4WHDENXstpignCuQDl+ngUUnzMfsBy1dWstl3vSXpngu1hgSAqYuwcBjiyR0hXvZIDqffvZDx3g==";
        };
        _FGJDceAq = {
            "id" = "FGJDceAq";
            "file" = "Cobweb Outline.zip";
            "hash" = "sha512-FJ9PmbbIUsg4WHDENXstpignCuQDl+ngUUnzMfsBy1dWstl3vSXpngu1hgSAqYuwcBjiyR0hXvZIDqffvZDx3g==";
        };
        _xgranraa = {
            "id" = "xgranraa";
            "file" = "Cobweb Outline.zip";
            "hash" = "sha512-FJ9PmbbIUsg4WHDENXstpignCuQDl+ngUUnzMfsBy1dWstl3vSXpngu1hgSAqYuwcBjiyR0hXvZIDqffvZDx3g==";
        };
        _9DwmjCBi = {
            "id" = "9DwmjCBi";
            "file" = "Cobweb Outline.zip";
            "hash" = "sha512-FJ9PmbbIUsg4WHDENXstpignCuQDl+ngUUnzMfsBy1dWstl3vSXpngu1hgSAqYuwcBjiyR0hXvZIDqffvZDx3g==";
        };
        _rdytGdBX = {
            "id" = "rdytGdBX";
            "file" = "Cobweb Outline.zip";
            "hash" = "sha512-FJ9PmbbIUsg4WHDENXstpignCuQDl+ngUUnzMfsBy1dWstl3vSXpngu1hgSAqYuwcBjiyR0hXvZIDqffvZDx3g==";
        };
        _Msn1svo0 = {
            "id" = "Msn1svo0";
            "file" = "Cobweb Outline.zip";
            "hash" = "sha512-FJ9PmbbIUsg4WHDENXstpignCuQDl+ngUUnzMfsBy1dWstl3vSXpngu1hgSAqYuwcBjiyR0hXvZIDqffvZDx3g==";
        };
        _JMUvs2Sp = {
            "id" = "JMUvs2Sp";
            "file" = "Cobweb Outline.zip";
            "hash" = "sha512-FJ9PmbbIUsg4WHDENXstpignCuQDl+ngUUnzMfsBy1dWstl3vSXpngu1hgSAqYuwcBjiyR0hXvZIDqffvZDx3g==";
        };
        _NjEGJjII = {
            "id" = "NjEGJjII";
            "file" = "Cobweb Outline.zip";
            "hash" = "sha512-FJ9PmbbIUsg4WHDENXstpignCuQDl+ngUUnzMfsBy1dWstl3vSXpngu1hgSAqYuwcBjiyR0hXvZIDqffvZDx3g==";
        };
        _DwNgDNCV = {
            "id" = "DwNgDNCV";
            "file" = "Cobweb Outline.zip";
            "hash" = "sha512-5Lc3bWY/M9v8j2inuF9V24uWnKXVp9IkkYKl6pZrMtJ3SlJfGIYdbMvz6Ws9SUkmSUXtnt35nDJ0NPB4bcPR2w==";
        };
        _s44YNB9q = {
            "id" = "s44YNB9q";
            "file" = "Cobweb Outline.zip";
            "hash" = "sha512-5Lc3bWY/M9v8j2inuF9V24uWnKXVp9IkkYKl6pZrMtJ3SlJfGIYdbMvz6Ws9SUkmSUXtnt35nDJ0NPB4bcPR2w==";
        };
        _HHCv01KR = {
            "id" = "HHCv01KR";
            "file" = "Cobweb Outline.zip";
            "hash" = "sha512-5Lc3bWY/M9v8j2inuF9V24uWnKXVp9IkkYKl6pZrMtJ3SlJfGIYdbMvz6Ws9SUkmSUXtnt35nDJ0NPB4bcPR2w==";
        };
        _68x940m3 = {
            "id" = "68x940m3";
            "file" = "Cobweb Outline.zip";
            "hash" = "sha512-d4zvQ2IRW85iSGll7WdbIvXkMJ9gq1iS6iP74jvDZtTvsAeOdCMzkYBUoaWXo8YV1DKODcPofMXKgrdcvnu+ng==";
        };
        _gl3eGhBL = {
            "id" = "gl3eGhBL";
            "file" = "Cobweb Outline.zip";
            "hash" = "sha512-okV/mRF0+yFH/v6pm0tFsBD17L5jJByIlAAnQvyLxDNRBbOFCB1mgYQknRHbfK/ATP44UVHf9Ye5MH4cLXhfLw==";
        };
        _Mfaof7FJ = {
            "id" = "Mfaof7FJ";
            "file" = "Cobweb Outline.zip";
            "hash" = "sha512-F3W95KURKmoxEwXRYh2mKV0jzIn1N9B4/FD1CQnWsh4NfK258AYSPmYV9RKrdfZdF61KEtwlcYMSe1cNnMF0Hg==";
        };
        _BbLdK6ne = {
            "id" = "BbLdK6ne";
            "file" = "Cobweb Outline.zip";
            "hash" = "sha512-F3W95KURKmoxEwXRYh2mKV0jzIn1N9B4/FD1CQnWsh4NfK258AYSPmYV9RKrdfZdF61KEtwlcYMSe1cNnMF0Hg==";
        };
        _3tEna9wn = {
            "id" = "3tEna9wn";
            "file" = "Cobweb Outline.zip";
            "hash" = "sha512-F3W95KURKmoxEwXRYh2mKV0jzIn1N9B4/FD1CQnWsh4NfK258AYSPmYV9RKrdfZdF61KEtwlcYMSe1cNnMF0Hg==";
        };
        _3C2oLMDJ = {
            "id" = "3C2oLMDJ";
            "file" = "Cobweb Outline.zip";
            "hash" = "sha512-WBMnmEmWB3O5+19x/XZwtZrp2xhGVp30RZBgesbMfzMwOA0NHejXMOUJu3cbvJBCrq5qLUJkPiEz10wen1D+kA==";
        };
    in {
        "nzKaGn0h" = _nzKaGn0h;
        "GqLacqjY" = _GqLacqjY;
        "w8ZNVjwr" = _w8ZNVjwr;
        "HtjGizUo" = _HtjGizUo;
        "mYnFHnfb" = _mYnFHnfb;
        "2WzmpetA" = _2WzmpetA;
        "QsSiZsU4" = _QsSiZsU4;
        "VxgTa8Mp" = _VxgTa8Mp;
        "YwH559by" = _YwH559by;
        "FGJDceAq" = _FGJDceAq;
        "xgranraa" = _xgranraa;
        "9DwmjCBi" = _9DwmjCBi;
        "rdytGdBX" = _rdytGdBX;
        "Msn1svo0" = _Msn1svo0;
        "JMUvs2Sp" = _JMUvs2Sp;
        "NjEGJjII" = _NjEGJjII;
        "DwNgDNCV" = _DwNgDNCV;
        "s44YNB9q" = _s44YNB9q;
        "HHCv01KR" = _HHCv01KR;
        "68x940m3" = _68x940m3;
        "gl3eGhBL" = _gl3eGhBL;
        "Mfaof7FJ" = _Mfaof7FJ;
        "BbLdK6ne" = _BbLdK6ne;
        "3tEna9wn" = _3tEna9wn;
        "3C2oLMDJ" = _3C2oLMDJ;
        "minecraft-1.19" = _nzKaGn0h;
        "minecraft-1.19.1" = _GqLacqjY;
        "minecraft-1.19.2" = _GqLacqjY;
        "minecraft-1.19.3" = _w8ZNVjwr;
        "minecraft-1.19.4" = _w8ZNVjwr;
        "minecraft-1.20" = _HtjGizUo;
        "minecraft-1.20.1" = _HtjGizUo;
        "minecraft-1.20.2" = _mYnFHnfb;
        "minecraft-1.20.3" = _mYnFHnfb;
        "minecraft-1.20.4" = _2WzmpetA;
        "minecraft-1.20.5" = _2WzmpetA;
        "minecraft-1.20.6" = _QsSiZsU4;
        "minecraft-1.21" = _Mfaof7FJ;
        "minecraft-1.21.1" = _Mfaof7FJ;
        "minecraft-1.21.2" = _Mfaof7FJ;
        "minecraft-1.21.3" = _Mfaof7FJ;
        "minecraft-1.21.4" = _Mfaof7FJ;
        "minecraft-1.21.5" = _Mfaof7FJ;
        "minecraft-1.21.6" = _Mfaof7FJ;
        "minecraft-1.21.7" = _Mfaof7FJ;
        "minecraft-1.21.8" = _Mfaof7FJ;
        "minecraft-1.21.9" = _Mfaof7FJ;
        "minecraft-1.21.10" = _Mfaof7FJ;
        "minecraft-1.21.11" = _Mfaof7FJ;
        "minecraft-26.1-snapshot-1" = _JMUvs2Sp;
        "minecraft-26.1-snapshot-2" = _JMUvs2Sp;
        "minecraft-26.1-snapshot-3" = _JMUvs2Sp;
        "minecraft-26.1-snapshot-4" = _JMUvs2Sp;
        "minecraft-26.1-snapshot-5" = _JMUvs2Sp;
        "minecraft-26.1-snapshot-6" = _JMUvs2Sp;
        "minecraft-26.1-snapshot-7" = _JMUvs2Sp;
        "minecraft-26.1-snapshot-8" = _JMUvs2Sp;
        "minecraft-26.1-snapshot-9" = _JMUvs2Sp;
        "minecraft-26.1" = _BbLdK6ne;
        "minecraft-26.1.1" = _BbLdK6ne;
        "minecraft-26.1.2" = _BbLdK6ne;
        "minecraft-26.2" = _3tEna9wn;
        "minecraft-26.3" = _3C2oLMDJ;
        "pkg-1.19" = _nzKaGn0h;
        "pkg-1.19.1-1.19.2" = _GqLacqjY;
        "pkg-1.19.3-1.19.4" = _w8ZNVjwr;
        "pkg-1.20-1.20.1" = _HtjGizUo;
        "pkg-1.20.2-1.20.3" = _mYnFHnfb;
        "pkg-1.20.4-1.20.5" = _2WzmpetA;
        "pkg-1.20.6" = _QsSiZsU4;
        "pkg-1.21" = _VxgTa8Mp;
        "pkg-1.21.1-1.21.2" = _YwH559by;
        "pkg-1.21.3-1.21.4" = _FGJDceAq;
        "pkg-1.21.5-1.21.6" = _xgranraa;
        "pkg-1.21.7-1.21.8" = _9DwmjCBi;
        "pkg-1.21.9-1.21.10" = _rdytGdBX;
        "pkg-1.21.11" = _HHCv01KR;
        "pkg-26.1-26.9" = _JMUvs2Sp;
        "pkg-26.1" = _NjEGJjII;
        "pkg-26.1.1" = _DwNgDNCV;
        "pkg-26.1.2" = _s44YNB9q;
        "pkg-1.21-1.21.11" = _Mfaof7FJ;
        "pkg-26.2" = _3tEna9wn;
        "pkg-26.1-26.1.2" = _BbLdK6ne;
        "pkg-26.3" = _3C2oLMDJ;
        "default" = _3C2oLMDJ;
    });
    fn = lib.prismnix.pkgs.mkVersionedModrinthPkgFn {
        name = "cobweb-outline";
        id = "akF2aZYV";
        type = "resourcepack";
        versions = versions;
        meta = {
            license = lib.getLicenseFromSpdxIdOr "MIT" {
                free = false;
                deprecated = false;
                redistributable = false;
                fullName = "MIT License";
                shortName = "MIT";
                url = null;
            };
        };
    };
in callPackage fn {}